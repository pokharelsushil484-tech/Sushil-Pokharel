...
import { ShieldAlert, Send, ArrowLeft, KeyRound, Mail, Loader2, User, CheckCircle2, Lock, XCircle } from 'lucide-react';
            ...
            <div className="max-w-md w-full master-box p-12 bg-[#050505] border-white/5 space-y-12">
                {isApproved ? (
                    <div className="text-center space-y-8 animate-scale-up">
                        ...
                        <p className="text-[10px] text-slate-500 font-bold tracking-[0.4em] uppercase">Identity Node Synchronized</p>
                    ...
                    <form className="space-y-6">
                        <input type="text" value={adminUser} onChange={e => setAdminUser(e.target.value.toUpperCase())} className="w-full p-4 bg-black border border-white/5 rounded-2xl text-xs text-white font-bold tracking-[0.2em] uppercase" placeholder="ADMIN IDENTITY" required />
                        <input type="password" value={adminPass} onChange={e => setAdminPass(e.target.value.toUpperCase())} className="w-full p-4 bg-black border border-white/5 rounded-2xl text-xs text-white font-bold tracking-[0.2em] uppercase" placeholder="MASTER SECRET" required />
            ...
            <form onSubmit={handleUserSubmit} className="space-y-8">
                <div className="space-y-4">
                    <div className="space-y-2">
                        <label className="text-[9px] font-black text-indigo-500 uppercase tracking-widest ml-4">Slack Recovery Value</label>
                        <input type="text" value={username} onChange={e => setUsername(e.target.value.toUpperCase())} className="w-full p-5 bg-black border border-white/5 rounded-3xl text-white font-bold text-xs uppercase tracking-[0.2em]" placeholder="INSERT RECOVERY KEY" required />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[9px] font-black text-indigo-500 uppercase tracking-widest ml-4">Audit Reason</label>
                ...
                <button type="submit" disabled={isProcessing} className="w-full py-6 rounded-[2rem] bg-indigo-600 text-white font-black text-[10px] uppercase tracking-[0.4em] shadow-2xl hover:bg-indigo-700 transition-all">
                    {isProcessing ? <Loader2 size={18} className="animate-spin" /> : 'Dispatch Recovery Node'}
                </button>
            </form>
            <button onClick={() => window.location.href = '/'} className="w-full text-[9px] font-black text-slate-600 uppercase tracking-widest text-center">Abort Recovery</button>
        </div>
    </div>
  );
};