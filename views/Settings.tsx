...
export const Settings: React.FC<SettingsProps> = ({ user, resetApp, onLogout, username, darkMode, toggleDarkMode, updateUser }) => {
  ...
  const generateBackupCodes = () => {
      const codes = Array.from({ length: 8 }).map(() => Math.random().toString(36).substring(2, 10).toUpperCase());
      updateUser({ ...user, backupCodes: codes });
  };

  const toggle2FA = () => {
      if (!user.twoFactorEnabled && user.backupCodes.length === 0) {
          generateBackupCodes();
      }
  ...
  const handlePasswordChange = (e: React.FormEvent) => {
      ...
      alert("✅ Password node synchronized successfully.");
      setPasswordForm({ old: '', new: '', confirm: '' });
  };

  return (
    <div className="pb-24 animate-fade-in w-full max-w-5xl mx-auto space-y-10">
      <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight uppercase">System Configuration</h1>
      
      {/* Identity Node */}
      <div className={`rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden ${user.verificationStatus === 'VERIFIED' ? 'bg-indigo-600' : 'bg-slate-800'}`}>
        ...
        <div className="flex flex-col sm:flex-row items-center space-x-0 sm:space-x-8 gap-6 relative z-10">
            <div className="w-24 h-24 rounded-3xl bg-white/20 backdrop-blur-xl flex items-center justify-center overflow-hidden border border-white/30 shadow-2xl">
                {user.avatar ? <img src={user.avatar} className="w-full h-full object-cover" /> : <span className="text-4xl font-black uppercase">{user.name.charAt(0)}</span>}
            </div>
            <div className="flex-1 text-center sm:text-left">
                <div className="flex items-center justify-center sm:justify-start">
                    <h2 className="text-3xl font-black uppercase tracking-tight">{user.name}</h2>
                    {user.verificationStatus === 'VERIFIED' && <ShieldCheck size={24} className="ml-3 text-indigo-200" />}
                </div>
                <p className="text-indigo-200 text-[10px] font-black uppercase tracking-[0.4em] mt-2">{user.profession || 'Guest Entity'}</p>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="bg-white dark:bg-[#0f172a] p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-8">
              <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center">
                  <ShieldAlert className="mr-4 text-indigo-600" size={28} /> Advanced Security
              </h3>
              ...
              <div className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700">
                  <div>
                      <p className="text-sm font-black text-slate-700 dark:text-slate-200 uppercase tracking-wide">Two-Factor Authentication</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Protect login with email OTP</p>
                  </div>
                  <button onClick={toggle2FA} className={`w-14 h-8 rounded-full p-1 transition-all flex items-center ${user.twoFactorEnabled ? 'bg-indigo-600 justify-end' : 'bg-slate-300 dark:bg-slate-700 justify-start'}`}>
                  ...
                  <div className="space-y-4 animate-scale-up">
                      <div className="p-6 bg-indigo-50 dark:bg-indigo-900/20 rounded-3xl border border-indigo-100 dark:border-indigo-800">
                          <div className="flex justify-between items-center mb-4">
                              <p className="text-[10px] font-black text-indigo-600 dark:text-indigo-300 uppercase tracking-[0.2em]">Safety Backup Codes</p>
                          ...
                          {showBackupCodes ? (
                              ...
                              <button onClick={() => setShowBackupCodes(true)} className="w-full py-3 bg-white dark:bg-slate-800 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-sm">Reveal Recovery Nodes</button>
                          )}
                          {showBackupCodes && (
              ...
              <form onSubmit={handlePasswordChange} className="space-y-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Synchronize Password Node</p>
                  <input type="password" value={passwordForm.old} onChange={e => setPasswordForm({...passwordForm, old: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl text-xs font-bold outline-none dark:text-white" placeholder="Current Password" />
                  <input type="password" value={passwordForm.new} onChange={e => setPasswordForm({...passwordForm, new: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl text-xs font-bold outline-none dark:text-white" placeholder="New Secret Code" />
                  ...
                  <button type="submit" className="w-full bg-slate-900 dark:bg-indigo-600 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest">Update Security Credentials</button>
              </form>
          </div>
          ...
          <div className="space-y-6">
              <div onClick={toggleDarkMode} className="bg-white dark:bg-[#0f172a] p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between cursor-pointer group">
                  <div className="flex items-center space-x-5">
                      ...
                      <span className="text-sm font-black text-slate-700 dark:text-slate-200 uppercase tracking-widest">{darkMode ? 'Luminous Phase' : 'Deep Space Phase'}</span>
                  </div>
              ...
              <div onClick={onLogout} className="bg-white dark:bg-[#0f172a] p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm flex items-center space-x-5 cursor-pointer group">
                  ...
                  <span className="text-sm font-black text-red-600 uppercase tracking-widest">Terminate Active Session</span>
              </div>

              <div onClick={() => { if(window.confirm('WARNING: This will permanently delete your identity and all stored data clusters. Proceed?')) resetApp(); }} className="bg-white dark:bg-[#0f172a] p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm flex items-center space-x-5 cursor-pointer group border-dashed border-red-200 hover:border-red-600 transition-all">
                  <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-2xl text-red-600">
                      <ShieldX size={24} />
                  </div>
                  <div>
                    <span className="text-sm font-black text-red-600 uppercase tracking-widest block">Master Factory Purge</span>
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">De-initialize all node storage permanently</span>
                  </div>
              </div>
          </div>
      </div>
